...
def run_feature_engineering_fork(train_data, test_file_path):
    # ---------------------------------------------
    # IMPORTS E SETUP
    # ---------------------------------------------
    import json
    ...
    def _move_stats_for_side(tl, who, window=None):
        ...
        for t in seq:
            ...
            if isinstance(pr,(int,float)): pr.append(float(pri))
    ...
    def _one_record_features(r):
        ...
        f["p1_offense_bias_ratio"] = (p1_sum_atk + 1e-3) / (p1_sum_spa + 1e-3)
        ...
        # Early HP volatility (first 5 turns)
        ...
        # Hazards effectiveness × switches
        ...
        # Extra global strength features from POKEMON_STATS
        try:
            winrates = [
    ...
    def add_advanced_top5_features(df: pd.DataFrame, name: str = "df") -> pd.DataFrame:
        ...
        print(f"[Advanced Top-5] Added 5 features to {name}: {new_cols2}")
        return df
...
# ============================================
# 3.1 - Best Features Selection (LR / XGB / RF)
# ============================================
import numpy as np
import pandas as pd
import xgboost as xgb
...
def run_feature_selection_fork(
    ...
    """
    Full feature-selection pipeline for Model 2.
    
    Steps:
      A) Remove constant features (GLOBAL: for all models)
      B) Correlation pruning (|ρ| > corr_threshold) — ONLY applied to the LR block
      C) Train 3 models to compute feature importance:
            * Logistic Regression (|coef|)
            * XGBoost (feature_importances_)
            * Random Forest (feature_importances_)
      D) Select top-K features for each model
      E) Build train/test reduced frames using the UNION of all selected features

    Returns:
      train_reduced, test_reduced,
      selected_cols_lr, selected_cols_xgb, selected_cols_rf,
      selected_cols_union, imp_df
    """

    assert target_col in train_df.columns, f"target_col '{target_col}' not found in train_df."
    ...
    # ------------------------------------------
    # A) Constant features removal (GLOBAL)
    # ------------------------------------------
    const_cols = [c for c in X0.columns if X0[c].nunique(dropna=True) <= 1]
    print(f"\n[Pruning][A] Constant features removed: {len(const_cols)}")
    ...
    print(f"[Pruning][A] After removal: {X_base.shape[1]} features")

    # Separated blocks for each method
    X_LR  = X_base.copy()
    X_XGB = X_base.copy()
    X_RF  = X_base.copy()

    # ------------------------------------------
    # B) Correlation pruning for LR block only
    # ------------------------------------------
    print(f"\n[Pruning][B] Correlation pruning for LR (|ρ| > {CORR_THRESHOLD}) ...")

    num_X_lr = X_LR.select_dtypes(include=[np.number])
    corr_matrix = num_X_lr.corr().abs()
    ...
    xgb_imp = {col: imp for col, imp in zip(X_XGB.columns, xgb_fs.feature_importances_)}

    # --- Random Forest ---
    print("[FS][RF ] Training RandomForest...")
    ...
    rf_imp = {col: imp for col, imp in zip(X_RF.columns, rf_fs.feature_importances_)}

    # --- Logistic Regression (L2) ---
    print("[FS][LR ] Training LogisticRegression...")
    scaler = StandardScaler()
    X_lr_std = scaler.fit_transform(X_LR_imp)
    ...
    lr_imp = {col: imp for col, imp in zip(X_LR.columns, np.abs(lr_fs.coef_[0]))}

    # ------------------------------------------
    # D) Build importance DataFrame
    # ------------------------------------------
    all_cols = sorted(X_base.columns)

    imp_df = pd.DataFrame({
        "feature": all_cols,
        "lr_imp":  [lr_imp.get(c, 0.0)  for c in all_cols],
        "xgb_imp": [xgb_imp.get(c, 0.0) for c in all_cols],
        "rf_imp":  [rf_imp.get(c, 0.0)  for c in all_cols],
    }).fillna(0.0)
    ...
    # ------------------------------------------
    def _top_k(df, col, k):
        return df.sort_values(col, ascending=False)["feature"].head(k).tolist()
    ...
    # Union of features
    selected_union = sorted(set(selected_cols_lr) |
                            set(selected_cols_xgb) |
                            set(selected_cols_rf))

    # Reduced train/test
    train_reduced = pd.concat(
        [train_df[ID_COLS + [TARGET_COL]], train_df[selected_union]],
        axis=1
    )
    test_reduced = pd.concat(
        [test_df[ID_COLS], test_df[selected_union]],
        axis=1
    )
...
# ============================================
# 3.2 - Stacking + Meta-search (Model 2)
# ============================================
import numpy as np
import pandas as pd
...
def run_stacking_and_meta_search(
    ...
    """
    Full stacking pipeline for Model 2:
      1) Base learners:
            - LogisticRegression (LR) on selected_cols_lr
            - XGBoost (XGB) on selected_cols_xgb
            - RandomForest (RF) on selected_cols_rf
         Each trained in K-fold CV to produce OOF probabilities.
      2) True stacking:
            - Meta-learner = LogisticRegression on [p_LR, p_XGB, p_RF]
            - First: OOF CV meta model (for diagnostics)
      3) Meta-search:
            - Grid over C & random_state for LogisticRegression on meta level
            - Select best combo by ROC-AUC (then accuracy)
            - Override final oof_meta_scores & meta_test_scores

    Returns:
        oof_meta_scores : np.ndarray, shape (n_train,)
        meta_test_scores: np.ndarray, shape (n_test,)
        y               : np.ndarray, training labels
    """

    assert target_col in train_reduced.columns, f"Target column '{target_col}' not found."
    y = train_reduced[target_col].astype(int).to_numpy()

    # Feature matrices for each base learner
    ...
    # --- OOF and test holders for base models --------------------------------
    n_base_for_meta = 3
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X_lr_full, y), 1):
        ...
        # ---- Base 2: XGBoost (early stopping) + sigmoid calibration ----
        ...
        # ---- Base 3: RandomForest + sigmoid calibration ----
    ...
    # --- Aggregate test probabilities per base learner -----------------------
    test_base_mean = test_base_folds.mean(axis=2)  # shape (n_test, 3)

    # --- First meta-learner (CV-based, mainly for diagnostics) ---------------
    ...
    # Fit meta on all OOF (diagnostic version)
    meta_clf.fit(oof_base, y)
    meta_test_scores = meta_test_folds.mean(axis=1)

    # --- OOF report for this meta predictor ----------------------------------
    ...
    # ======================================================================
    # Meta-search: small grid over (C, random_state) on meta level
    # ======================================================================
    X_meta = np.asarray(oof_base, dtype=float)         # (n_train, 3)
    X_meta_test = np.asarray(test_base_mean, dtype=float)  # (n_test, 3)
    ...
    # Rank configs by AUC, then Accuracy
    ...
    print(
        f"\n[Meta-search] Selected best meta config: "
        f"{best_key} (ACC={best_acc:.4f}, AUC={best_auc:.4f})"
    )

    # Override meta scores with best configuration
    oof_meta_scores = meta_oof_candidates[best_key]
    meta_test_scores = meta_test_candidates[best_key]

    print(
        "\n[Meta-search] Updated 'oof_meta_scores' and 'meta_test_scores' "
        "with the best meta configuration."
    )
    print("Ready for threshold tuning with these improved meta predictions.")

    return oof_meta_scores, meta_test_scores, y

# ====================================
# 3.3 + 4 — Threshold tuning + final submission
# ====================================
import numpy as np
import pandas as pd
...
def run_threshold_tuning_and_submission(
    oof_meta_scores: np.ndarray,
    meta_test_scores: np.ndarray,
    y: np.ndarray,
    test_df: pd.DataFrame,
    ...
    Full 3.3 + 4 pipeline:
      ...
      1) Advanced threshold tuning on OOF predictions:
         - Scan thresholds in [thr_min, thr_max]
         - Compute accuracy, F1, MCC, AUC, logloss
         - Pick the final operating threshold:
             * maximize Accuracy
             * break ties using F1, then MCC

      2) Final OOF report at the selected threshold.

      3) Build test predictions, create the submission DataFrame
         and save it to CSV (default: 'submission.csv').

    Args
    ----
    oof_meta_scores : np.ndarray
        Out-of-fold probabilities from the stacking meta-learner (train size).
    meta_test_scores: np.ndarray
        Test probabilities from the stacking meta-learner (test size).
    y               : np.ndarray
        Training labels (0/1).
    test_df         : pd.DataFrame
        Test DataFrame containing at least 'battle_id'.
    submission_filename : str
        Output CSV file name (default 'submission.csv').
    thr_min, thr_max, thr_points : float/int
    ...
    result : dict
        {
            ...
            "thr_df": pd.DataFrame,          # scan of thresholds and metrics
            "oof_metrics": dict,             # final metrics at best threshold
            "submission": pd.DataFrame,      # final submission
            "stack_pred_labels_tuned": np.ndarray,
        }
    """

    # --- Basic checks ---
    if "battle_id" not in test_df.columns:
        raise ValueError("test_df must contain a 'battle_id' column for submission.")
    ...
    # --- Scan thresholds ---
    thr_values = np.linspace(thr_min, thr_max, thr_points)
    rows = []
    ...
    # --- Main operating threshold: prefer Accuracy, tie-break with F1 then MCC ---
    best_row = best_acc_row.copy()

    # If F1-optimal threshold has same accuracy within 1e-4 but better F1, take that
    if (
        abs(best_f1_row["accuracy"] - best_row["accuracy"]) < 1e-4
        and best_f1_row["f1"] > best_row["f1"]
    ):
        best_row = best_f1_row

    # Similarly check MCC if still tied
    if (
        abs(best_mcc_row["accuracy"] - best_row["accuracy"]) < 1e-4
        and best_mcc_row["mcc"] > best_row["mcc"]
    ):
    ...
    # Alias for compatibility with the original notebook idea
    ...
    try:
        from IPython.display import display  # works if running in notebook
        display(submission.head())
    except Exception:
        print(submission.head())

    # --- Save CSV exactly as in the original notebook (step 4) ---
    submission.to_csv(submission_filename, index=False)
    print(f"\n[4] Submission file '{submission_filename}' created.")
    print("[4] Submission shape:", submission.shape)

    # For convenience in memory (similar to original globals)
    stacking_best_threshold = best_threshold
    stacking_submission = submission.copy()

    return {
        "best_threshold": stacking_best_threshold,
        "thr_df": thr_df,
        "oof_metrics": oof_metrics,
        "submission": stacking_submission,
        "stack_pred_labels_tuned": stack_pred_labels_tuned,
    }
