...
# =========================
# Cell 2 — Feature Engineering for Model 3
# =========================
...
from typing import List, Dict, Any, Tuple
from collections import Counter, defaultdict
from IPython.display import display  # safe in notebooks, no-op in scripts
...
def _stab_features(record: Dict[str, Any], max_turns: int = 30) -> Dict[str, float]:
    ...
    def _accumulate(seq):
        p1_total = p1_stab = 0
        p2_total = p2_stab = 0  # p2 not modeled, kept for symmetry
...
# ---------------------------------------------
# NEW FEATURES (simple HP + team features)
# ---------------------------------------------
def new_features(r):
...
# ---------------------------------------------
# Global stats placeholders (populated inside run_feature_engineering_mrk)
# ---------------------------------------------
POKEMON_STATS    = {}
POKEMON_HP_STATS = {}
pokemon_avg_damage = {}
...
# ---------------------------------------------
# Full per-record feature builder
# ---------------------------------------------
def _one_record_features(r: Dict[str, Any]) -> Dict[str, float]:
    ...
    # --- New bounded team / HP volatility features ---
    ...
    # Matchup / switches / hazards / momentum / recovery / STAB / early / priority / custom blocks
    ...
    # Team Winrate / HP resilience / Avg damage (use globals filled inside run_feature_engineering_mrk)
    try:
...
# ---------------------------------------------
# Public helper: build full feature DataFrame
# ---------------------------------------------
...
def run_feature_engineering_mrk(
    ...
    """
    Run the full feature engineering pipeline for Model 3.

    Parameters
    ----------
    train_data : list of dict
        Parsed JSONL records for the training set (each element is a battle).
    test_file_path : str
        Path to the JSONL file with test battles.
    alpha : float
        Smoothing parameter for the Pokémon win-rate stats.

    Returns
    -------
    train_df : pd.DataFrame
        Clean, numeric feature matrix for training (includes 'player_won').
    test_df : pd.DataFrame
        Clean, numeric feature matrix for test (no target).
    train_df_raw : pd.DataFrame
        Copy of train_df before custom 2.x engineered features.
    test_df_raw : pd.DataFrame
        Copy of test_df before custom 2.x engineered features.
    """
    global POKEMON_STATS, POKEMON_HP_STATS, pokemon_avg_damage

    # Build global stats used inside _one_record_features
    ...
    # ---------------------------------------------
    # 1) Base feature extraction (static + timeline + Mirko/Deb blocks)
    # ---------------------------------------------
    ...
    # ---------------------------------------------
    # 1.b) Manual interactions
    # ---------------------------------------------
    train_df = _maybe_add_interactions(train_df)
    test_df  = _maybe_add_interactions(test_df)

    # Keep raw copies (before custom 2.x engineered features)
    train_df_raw = train_df.copy()
    test_df_raw  = test_df.copy()

    # ---------------------------------------------
    # 1.c) Basic numeric cleaning
    # ---------------------------------------------
    ...
    # ---------------------------------------------
    # 2.x Custom predictive features (10 high-signal ratios)
    # ---------------------------------------------
